...
USBH_ClassTypeDef CUSTOM_AUDIO_Class =
    {
        "AUDIO",
        AC_CLASS,
...
static USBH_StatusTypeDef USBH_AUDIO_CSRequest(USBH_HandleTypeDef *phost,
    ...
    // printf("USBH_AUDIO_CSRequest cs_req_state %d\n", AUDIO_Handle->cs_req_state);
...
static USBH_StatusTypeDef USBH_AUDIO_Process(USBH_HandleTypeDef *phost) {
    ...
    // printf("PROCESS %d\n", AUDIO_Handle->microphone.supported);
...
static USBH_StatusTypeDef USBH_AUDIO_BuildMicrophonePath(USBH_HandleTypeDef *phost) {
    ...
    for (terminalIndex = 0U; terminalIndex < AUDIO_Handle->class_desc.InputTerminalNum; terminalIndex++) {
        if (LE16(AUDIO_Handle->class_desc.cs_desc.InputTerminalDesc[terminalIndex]->wTerminalType) == 0x201) {
            // printf("SAMPLE %d,%d,%d,%d,%d,", AUDIO_Handle->class_desc.as_desc->FormatTypeDesc->tSamFreq[0][1], AUDIO_Handle->class_desc.as_desc->FormatTypeDesc->tSamFreq[1][1], AUDIO_Handle->class_desc.as_desc->FormatTypeDesc->tSamFreq[2][1], AUDIO_Handle->class_desc.as_desc->FormatTypeDesc->tSamFreq[3][1], AUDIO_Handle->class_desc.as_desc->FormatTypeDesc->tSamFreq[4][1]);
...
static USBH_StatusTypeDef USBH_AUDIO_InputStream(USBH_HandleTypeDef *phost) {
    ...
    if (audio_flag == 1) {
        ...
        status = USBH_IsocReceiveData(phost, AUDIO_Handle->microphone.buf, AUDIO_Handle->microphone.frame_length, AUDIO_Handle->microphone.Pipe);

        // Debug print PCM samples
        int16_t *samples = (int16_t *)AUDIO_Handle->microphone.buf;
        // int16_t *samples = (int16_t *)AUDIO_Handle->microphone.audioInBuffer;
...
USBH_StatusTypeDef USBH_AUDIO_SetFrequency(USBH_HandleTypeDef *phost,
    ...
    // printf("FRAME LENGTH %d\n", (SampleRate * BitPerSample * NbrChannels) / 8000U);
    AUDIO_Handle->microphone.frequency = SampleRate;
    AUDIO_Handle->microphone.frame_length = (SampleRate * BitPerSample * NbrChannels) / 8000U;
